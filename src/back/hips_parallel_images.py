...
def extract_region_from_fits_header(
    fits_file: Path,
                ...
                if hdu.data is not None and hdu.header:
                    try:
                        ...
                        if wcs.has_celestial:
                            ...
                            if (
                                isinstance(corners_world, tuple)
                                and len(corners_world) == 2
                            ):
                                # Resultado como tupla (ra_array, dec_array)
                                ra_coords = np.array(corners_world[0])
                                dec_coords = np.array(corners_world[1])
                            elif (
                                hasattr(corners_world, "shape")
                                and len(corners_world.shape) == 2
                            ):
                            ...
                            ra_min, ra_max = float(np.min(ra_coords)), float(
                                np.max(ra_coords)
                            )
                            dec_min, dec_max = float(np.min(dec_coords)), float(
                                np.max(dec_coords)
                            )

                            return float(format(ra_min, '.4f')), float(format(ra_max, '.4f')), float(format(dec_min, '.4f')), float(format(dec_max, '.4f'))
...
def extract_image_id_from_filename(fits_file: Path, band, id_pattern: str = "auto") -> str:
    """
    ...
    # Fallback
    return f'{band}_{fits_file.stem.replace(".", "_").replace("-", "_")}'


def create_config_for_image(
    base_config: Dict,
    fits_file: Path,
    image_id: str,
    output_dir: Path,
    ...
    with open(config_path, "w", encoding="utf-8") as f:
        ...
        for key, value in base_config.items():
            if key == "input_dir":
                ...
                # Adiciona template local se especificado
                if value and Path(value).exists():
                    f.write(f'template="{value}"\n')
            elif key == "runs":
                continue
            else:
                f.write(f'{key}="{value}"\n')

        f.write('mode="mergeKeep"\n')
...
def create_consolidation_config(
    base_config: Dict, image_outputs: List[str], output_dir: Path, final_output: str
) -> Path:
...
def main():
    ...
    parser.add_argument(
        "images_pattern", help="Padrão glob para encontrar imagens FITS"
    )
    parser.add_argument(
        "--id-pattern",
        default="auto",
        choices=["auto", "tile", "coords"],
        help="Padrão para extrair ID da imagem",
    )
    parser.add_argument(
        "--max-images", type=int, help="Máximo de imagens por banda a processar"
    )
    parser.add_argument(
    ...
    for band, color in band_to_color.items():
        ...
        for fits_file in images:
            image_id = extract_image_id_from_filename(fits_file, band, args.id_pattern)

            region_bounds = extract_region_from_fits_header(fits_file)
            if region_bounds:
                ra_min, ra_max, dec_min, dec_max = region_bounds
                print(
                    f"  {image_id}: RA[{ra_min:.4f},{ra_max:.4f}] DEC[{dec_min:.4f},{dec_max:.4f}]"
                )
            else:
                print(f"  {image_id}: Erro. Não foi possível extrair região WCS")
                return 1
            ...
            # image_output = str(
            #     (cwd / f"{color_config['output_dir']}").absolute()
            # )
    ...
    for color in ["red", "green", "blue"]:
        ...
        final_outputs[f"in{color.capitalize()}"] = str(
            Path(cwd, final_output).absolute()
        )
    ...
    if consolidate_jobs:
        ...
        if not args.dry_run:
            final_job_id = submit_slurm_job(
                "rgb.sbatch", rgb_config_path, cwd, aladin_cmd, max_mem, dependency
            )
