...
def extract_region_id_from_filename(
    file_path: Path, color: str, id_pattern: str = "auto"
) -> str:
    """
...
def main():
    ...
    if args.max_images:
        for band, images in images_by_band.items():
            if images and len(images) > args.max_images:
    ...
    for band, color in band_to_color.items():
        ...
        for partition_file in images:
            region_id = extract_region_id_from_filename(
                partition_file, color, args.id_pattern
            )
